...
if (isset($_POST['save_exp'])) {
    ...
    if (empty($errors) == true) {
        ...
        $query2 = "INSERT INTO `basic` (id, dept, ddept, design, role, doj) VALUES ('$id','$dept','$ddept','$design','$role','$doj')";
...
if (isset($_GET['student_id2'])) {
    ...
    if (mysqli_num_rows($query_run) == 1) {
        ...
        $student['doj'] = date('Y-m-d', strtotime($student['doj']));
...
if (isset($_POST['update_faculty'])) {
    ...
    $query_run = mysqli_query($db, $query);
    $row = mysqli_fetch_assoc($query_run);
    $existing_cert = $row['cert'];

    // Initialize file path variable
    $filePath = $existing_cert; // Keep existing path by default

    // Check if new file is uploaded
    if (isset($_FILES['cert']) && $_FILES['cert']['size'] > 0) {
        $file_name = $_FILES['cert']['name'];
        $file_tmp = $_FILES['cert']['tmp_name'];
        $ext = pathinfo($file_name, PATHINFO_EXTENSION);
        $new_file_name = $name . $id . "." . $ext;
        $filePath = "images/profile/" . $new_file_name;

        // Delete old file if exists
        if (!empty($existing_cert) && file_exists($existing_cert)) {
            unlink($existing_cert);
        }
    ...
    if ($query_run) {
        ...
        $res = [
            'status' => 500,
            'message' => 'Details Not Updated: ' . mysqli_error($db)
        ];
        echo json_encode($res);
        return;
    }
